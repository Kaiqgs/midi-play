...
impl TrackManager {
    ...
    pub fn set_track(
        &mut self,
        game_track: Option<&mut GameTrack>,
        reutil: RenderUtil,
        peripheral: MidiPeripheral,
        fs: &Filesystem,
    ) -> Result<TrackWindowContext, GameError> {
        if game_track.is_none() {
            self.filepath = None;
            self.sheet_track.component_data.playback = peripheral;
            return Ok(TrackWindowContext::new(0, None, None));
        }
        let game_track = game_track.unwrap();
        let mut sheet_track = game_track.load_track(fs)?;
        // let path = Path::new(&str_path);
        // let mut sheet_track = self.parser.parse(path, fs)?;
        let render_range = sheet_track.compute_render_range(reutil);
        let ctx = TrackWindowContext::new(sheet_track.track_pairs.len(), None, Some(render_range));
